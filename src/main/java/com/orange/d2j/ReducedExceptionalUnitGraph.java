 ...
 */
public class ReducedExceptionalUnitGraph extends UnitGraph implements ExceptionalGraph<Unit>
{
	 ...
	 */
	public ReducedExceptionalUnitGraph(Body body, ThrowAnalysis throwAnalysis) {
		super(body);
		initialize(throwAnalysis);
	}
	 ...
	 *
	 */
	public ReducedExceptionalUnitGraph(Body body) {
	 ...
	 *		                     constructor.
	 */
	protected ReducedExceptionalUnitGraph(Body body, boolean ignoredBogusParameter) {
		super(body);
	}
	...
	protected Map<Unit,Collection<ExceptionDest>> buildExceptionDests(ThrowAnalysis throwAnalysis) {
		...
		for (Iterator<Trap> trapIt = body.getTraps().iterator(); trapIt.hasNext(); ) {
			...
			boolean isThrowable = catcher.getClassName().equals("java.lang.Throwable");
			...
			unitIt.hasNext(); ) {
				...
				if (catchableAs.getCaught() != ThrowableSet.Manager.v().EMPTY ||
						(isThrowable && throwAnalysis.mightThrow(unit) != ThrowableSet.Manager.v().EMPTY)) {
	 ...
	 *                    ReducedExceptionalUnitGraph.ExceptionDest
		...
		it.hasNext(); ) {
			...
			for (Iterator<ExceptionDest> destIt = dests.iterator(); destIt.hasNext(); ) {
				...
				if (dest.getTrap() != null) {
					...
					// Check the second part meaning May be forced to false.
					if (selfThrowables != null &&  selfThrowables.catchableAs(trapsType)) {
	...
	List <Unit> heads, tails;
	@Override
	public List<Unit> getHeads() { return heads; }
	@Override
	public List<Unit> getTails() { return tails; }


	/**
	 * Returns a collection of 
	 * {@link ReducedExceptionalUnitGraph.ExceptionDest ExceptionDest}
	...
	/**
	 * Representation of Exception targets
	 *
	 */
